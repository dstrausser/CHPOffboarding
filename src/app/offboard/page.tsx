    ...
    <div>
        ...
        {userDetails && (
          <div className="mt-4 bg-gray-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div><p className="font-medium text-gray-900">{userDetails.displayName}</p><p className="text-sm text-gray-500">{userDetails.userPrincipalName}</p></div>
              <StatusBadge status={userDetails.accountEnabled ? "success" : "error"} label={userDetails.accountEnabled ? "Active" : "Disabled"} />
            </div>
            {userDetails.licenseDetails && userDetails.licenseDetails.length > 0 && (
              <div className="mt-3"><p className="text-sm font-medium text-gray-700">Assigned Licenses:</p><div className="mt-1 flex flex-wrap gap-2">{userDetails.licenseDetails.map((lic) => <span key={lic.skuId} className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-md">{lic.skuPartNumber}</span>)}</div></div>
            )}
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {steps.convertToSharedMailbox && (
                <div className="ml-7 mt-3 bg-gray-50 rounded-lg p-4">
                  <p className="text-sm font-medium text-gray-700 mb-2">Add delegates to shared mailbox:</p>
                  <div className="flex gap-2 mb-2"><input type="email" value={newDelegate} onChange={(e) => setNewDelegate(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addDelegate()} placeholder="delegate@company.com" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" /><button onClick={addDelegate} className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Add</button></div>
                  {mailboxDelegates.length > 0 && <div className="flex flex-wrap gap-2">{mailboxDelegates.map((d) => <span key={d} className="inline-flex items-center gap-1 bg-white px-3 py-1 rounded-full text-sm border border-gray-200">{d}<button onClick={() => setMailboxDelegates(mailboxDelegates.filter((x) => x !== d))} className="text-red-400 hover:text-red-600 ml-1">x</button></span>)}</div>}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {steps.breakInheritance && (
                <div className="ml-7 mt-3 bg-gray-50 rounded-lg p-4">
                  <p className="text-sm font-medium text-gray-700 mb-2">Users who should have access to archived files:</p>
                  <div className="flex gap-2 mb-2"><input type="email" value={newViewer} onChange={(e) => setNewViewer(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addViewer()} placeholder="viewer@company.com" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" /><button onClick={addViewer} className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Add</button></div>
                  {archiveViewers.length > 0 && <div className="flex flex-wrap gap-2">{archiveViewers.map((v) => <span key={v} className="inline-flex items-center gap-1 bg-white px-3 py-1 rounded-full text-sm border border-gray-200">{v}<button onClick={() => setArchiveViewers(archiveViewers.filter((x) => x !== v))} className="text-red-400 hover:text-red-600 ml-1">x</button></span>)}</div>}
                </div>
              )}
            </div>
          </div>
      ...
      {selectedUser && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <button onClick={runOffboarding} disabled={running} className="w-full py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
            {running ? <span className="flex items-center justify-center gap-2"><span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" />Running Offboarding...</span> : `Offboard ${selectedUser.displayName}`}
          </button>
        </div>
      )}
      ...
      {results && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Offboarding Results</h2>
          <div className="space-y-3">{results.map((r, i) => <div key={i} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg"><StatusBadge status={r.status} /><div><p className="font-medium text-gray-900">{r.step}</p><p className="text-sm text-gray-600">{r.message}</p></div></div>)}</div>
        </div>
      )}
    </div>
  );
