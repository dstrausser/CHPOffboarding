...
export default function MailboxPage() {
  ...
  const addDelegate = async () => {
    ...
    setAdding(true);
    try {
      const res = await fetch("/api/mailbox/delegates", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mailboxUserId: selectedMailbox.id, delegateEmail }),
      });
      ...
      setDelegates([...delegates, { email: delegateEmail, status: "error", message: err.message || "Failed" }]);
    } finally {
      setAdding(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        {selectedMailbox && (
          <div className="mt-4 bg-green-50 rounded-lg p-4">
            <p className="font-medium text-green-900">Selected: {selectedMailbox.displayName}</p>
            <p className="text-sm text-green-700">{selectedMailbox.mail || selectedMailbox.userPrincipalName}</p>
          </div>
        )}
      </div>
      ...
      {selectedMailbox && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={addDelegate} disabled={adding || !delegateEmail} className="px-6 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              {adding ? <span className="flex items-center gap-2"><span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" />Adding...</span> : "Add Delegate"}
            </button>
          </div>
          {delegates.length > 0 && (
            <div className="mt-4 space-y-2">
              ...
              {delegates.map((d, i) => (
                <div key={i} className={`flex items-center justify-between p-3 rounded-lg ${d.status === "success" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                  <span className={`text-sm font-medium ${d.status === "success" ? "text-green-800" : "text-red-800"}`}>{d.email}</span>
                  <span className={`text-xs ${d.status === "success" ? "text-green-600" : "text-red-600"}`}>{d.message}</span>
                </div>
              ))}
            </div>
          )}
